...
echo.
echo  ╔══════════════════════════════════════════════╗
echo  ║        ELLIOT — Autonomous Web Auditor      ║
...
if not exist "%ROOT%\.venv\Scripts\python.exe" (
    ...
    echo         Then: .venv\Scripts\pip install -r elliot\requirements.txt
    pause
    exit /b 1
)
echo        Python venv .......... OK
...
:: Verify .env exists
if exist "%ROOT%\elliot\.env" (
    echo        elliot/.env ......... OK
) else (
    echo        [WARN] elliot/.env not found — API keys may be missing
)

:: Start Backend (FastAPI)
echo [3/5] Starting Backend (FastAPI on port 8000)...
start "Elliot Backend" cmd /k "cd /d "%ROOT%\backend" && "%ROOT%\.venv\Scripts\uvicorn.exe" main:app --host 0.0.0.0 --port 8000"
...
if !TRIES! GEQ 30 (
    ...
    echo        Check the "Elliot Backend" terminal window for errors.
    pause
    exit /b 1
)
...
start "Elliot Frontend" cmd /k "cd /d "%ROOT%\frontend" && npm run dev"
