...
export function TrustGauge({ score, size = 200, animate = true }: TrustGaugeProps) {
	// Validate score - fallback to 0 if invalid
	const validScore = typeof score === "number" && !isNaN(score) ? Math.min(Math.max(score, 0), 100) : 0;

	const [displayScore, setDisplayScore] = useState(animate ? 0 : validScore);
	const color = getColor(validScore);
	const label = getLabel(validScore);
	...
	useEffect(() => {
		...
		const tick = (now: number) => {
			...
			setDisplayScore(Math.round(validScore * eased));
