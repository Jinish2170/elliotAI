...
must_haves:
  truths:
    - SQLite database file exists in data/elliot_audits.db on first backend startup
...
Output: elliot/db/ package with models, config, and initialization; SQLite database file created on first run.
</objective>
...
<task type="auto">
  ...
  <action>
    Create the elliot/db package with three files:

    1. **elliot/db/config.py**:
       - Set DATABASE_URL = "sqlite+aiosqlite:///./data/elliot_audits.db"
       - Create declarative_base() instance as Base
       - Include constants for PRAGMA settings (SYNCHRONOUS, CACHE_SIZE, WAL_AUTO_CHECKPOINT)

    2. **elliot/db/models.py**:
  ...
  <verify>
    Run python -c "from elliot.db import init_database, Base, models; print('Models loaded:', [m.__name__ for m in Base.__subclasses__()])" to verify models import correctly.
  </verify>
  <done>
...
<verification>
1. Verify models import: Run `python -c "from elliot.db.models import Audit, AuditFinding, AuditScreenshot, AuditEvent; print('All models imported')"`
2. Verify configuration: Run `python -c "from elliot.db.config import DATABASE_URL, Base; print('DB URL:', DATABASE_URL)"`
3. Verify initialization: Run `python -c "import asyncio; from elliot.db import init_database; asyncio.run(init_database()); print('Database initialized')"` - this should create data/elliot_audits.db
4. Verify table structure: Run `sqlite3 data/elliot_audits.db ".schema audits"` to confirm audits table exists with WAL mode (check sqlite3 data/elliot_audits.db "PRAGMA journal_mode")
</verification>

<success_criteria>
- elliot/db package created with 3 files
...
- SQLite database file created in data/elliot_audits.db on first initialization
