...
must_haves:
  ...
  artifacts:
    ...
    - path: "backend/routes/audit.py"
      ...
      exports: ["DbSession"]
  key_links:
    - from: "elliot/db/repositories.py"
      to: "elliot/db/models.py"
      via: "import Audit, AuditFinding, etc."
      pattern: "from elliot.db.models import"
    - from: "backend/routes/audit.py"
      to: "elliot/db/__init__.py"
      via: "import get_db for dependency injection"
      pattern: "from elliot.db import get_db"
    - from: "backend/routes/audit.py"
      to: "elliot/db/repositories.py"
      via: "import AuditRepository"
      pattern: "from elliot.db.repositories import"
---
...
Output: elliot/db/repositories.py with AuditRepository; backend/routes/audit.py updated with DbSession dependency.
</objective>
...
<task type="auto">
  ...
  <action>
    Create elliot/db/repositories.py with AuditRepository class:

    1. Import from elliot.db: AsyncSession
    2. Import from elliot.db.models: Audit, AuditFinding, AuditScreenshot, AuditStatus
  ...
  <verify>
    Run python -c "from elliot.db.repositories import AuditRepository; print('Repository imported:', hasattr(AuditRepository, 'get_by_id'))"
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
    ...
    1. Add import at top:
       ...
       - from elliot.db import get_db, AsyncSession
...
<verification>
1. Verify repository methods: Run `python -c "from elliot.db.repositories import AuditRepository; import inspect; print([m for m in dir(AuditRepository) if not m.startswith('_')])"`
