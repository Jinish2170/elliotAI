...
must_haves:
  ...
  key_links:
    - from: elliot/agents/security_agent.py
      to: elliot/analysis/security/utils.py
    ...
    - from: elliot/agents/security_agent.py
      to: elliot/agents/graph_investigator.py
...
From elliot/core/types.py (check if SecurityFinding/SecurityResult exist):
```python
...
From elliot/core/orchestrator.py (Phase 9 pattern):
...
<task type="auto">
  ...
  <files>elliot/core/types.py</files>
  <action>
    ...
    3. Import SecurityFinding and SecurityResult from core.types for use in security modules:
       - If they exist in core/types, update elliot/analysis/security/base.py to import from core.types
...
<task type="auto">
  ...
  <files>elliot/agents/security_agent.py</files>
  <action>
    ...
    4. Private method _calculate_cvss_scores(findings: List[SecurityFinding]) -> List[SecurityFinding]:
       - Import CVSSCalculator from elliot.cwe.cvss_calculator
       - For each finding:
...
<task type="auto">
  ...
  <files>elliot/agents/security_agent.py, elliot/core/orchestrator.py</files>
  <action>
    ...
    1. In SecurityAgent, add private method _correlate_darknet_threats(url: str, findings: List[SecurityFinding]) -> List[SecurityFinding]:
       - Import DarknetThreatIntel from elliot.agents.graph_investigator (or OSINTOrchestrator)
    ...
    3. Update elliot/core/orchestrator.py:
