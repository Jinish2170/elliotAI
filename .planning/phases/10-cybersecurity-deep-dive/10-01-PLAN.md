...
files_modified:
  ...
  - elliot/analysis/security/__init__.py
  - elliot/analysis/security/utils.py
  - elliot/analysis/security/tls_ssl.py
  - elliot/analysis/security/cookies.py
  - elliot/analysis/security/csp.py
...
must_haves:
  ...
  artifacts:
    ...
    - path: elliot/analysis/security/tls_ssl.py
      provides: TLS SSL certificate analysis, security header checking
      exports: ["SSLCertificateAnalyzer", "SecurityHeaderAnalyzer"]
    - path: elliot/analysis/security/cookies.py
      provides: Cookie security analysis (Secure, HttpOnly, SameSite)
      exports: ["CookieSecurityAnalyzer"]
    - path: elliot/analysis/security/csp.py
      provides: Content Security Policy validation
      exports: ["ContentSecurityPolicyAnalyzer"]
    - path: elliot/config/security_rules.py
      provides: CWE mappings, severity rules, rule configuration
      exports: ["cwemapper", "cvss_calculator"]
  key_links:
    - from: elliot/analysis/security/base.py
      to: elliot/cwe/cvss_calculator.py
      via: import elliot.cwe.cvss_calculator
      pattern: "from elliot.cwe.cvss_calculator import CVSSCalculator"
    - from: elliot/analysis/security/base.py
      to: elliot/cwe/registry.py
      via: import elliot.cwe.registry
      pattern: "from elliot.cwe.registry import CWERegistry"
    - from: elliot/analysis/security/utils.py
...
From elliot/cwe/cvss_calculator.py:
```python
...
<task type="auto">
  ...
  <files>elliot/analysis/security/base.py, elliot/analysis/security/__init__.py</files>
  <action>
    Create base.py with:
  ...
  <verify>
    <automated>python -c "from elliot.analysis.security import SecurityModule, SecurityFinding, SecurityTier; SecurityTier.FAST; print('Base class imports work')"</automated>
  </verify>
  <done>SecurityModule base class with SecurityTier enum and SecurityFinding dataclass (extended from core/types.py if exists) created in elliot/analysis/security/base.py</done>
</task>

<task type="auto">
  <name>Task 2: Create security utilities for module discovery and tier grouping</name>
  <files>elliot/analysis/security/utils.py, elliot/config/security_rules.py</files>
  <action>
    Create utils.py with:

    1. get_all_security_modules() function:
       - Uses importlib to discover all SecurityModule subclasses in elliot.analysis.security
    ...
    1. Import CWEMapper and CVSSCalculator from Phase 9:
       - from elliot.cwe.registry import CWERegistry
       - from elliot.cwe.cvss_calculator import CVSSCalculator
  ...
  <verify>
    <automated>python -c "from elliot.analysis.security.utils import get_all_security_modules, group_modules_by_tier; from elliot.config.security_rules import cwemapper, cvss_calculator; print('Utility imports work'); modules = get_all_security_modules(); print(f'Found {len(modules)} security modules')"</automated>
  </verify>
...
<task type="auto">
  ...
  <files>elliot/analysis/security/tls_ssl.py, elliot/analysis/security/cookies.py, elliot/analysis/security/csp.py</files>
  <action>
  ...
  <verify>
    <automated>python -c "from elliot.analysis.security.tls_ssl import SecurityHeaderAnalyzerEnhanced; from elliot.analysis.security.cookies import CookieSecurityAnalyzer; from elliot.analysis.security.csp import ContentSecurityPolicyAnalyzer; a = SecurityHeaderAnalyzerEnhanced(); c = CookieSecurityAnalyzer(); p = ContentSecurityPolicyAnalyzer(); print(f'{a.category_id}, {c.category_id}, {p.category_id}')"</automated>
  </verify>
