...
must_haves:
  truths:
    - "User can import SecurityAgent class from elliot.agents"
    - "User can import SecurityResult, SecurityFinding, SecurityConfig, Severity from elliot.core"
...
Create `elliot/core/types.py` with:
...
Create `elliot/agents/security_agent.py` with:
...
Modify `elliot/config/settings.py`:
...
### Task: Update imports
Modify `elliot/agents/__init__.py` and `elliot/core/__init__.py`:
