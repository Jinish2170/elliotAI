...
depends_on:
  - 03-01
files_modified:
  - elliot/tests/langgraph_investigation/test_02_full_audit_mocked.py
autonomous: true
requirements:
  - CORE-03
  - CORE-03-2
...
must_haves:
  ...
  key_links:
    - from: test_02_full_audit_mocked.py
      to: elliot.core.orchestrator.build_audit_graph
...
<objective>
Create full audit test with mocked NIMClient and agents to observe real execution path of ELLIOT StateGraph via ainvoke(), comparing with sequential execution fallback.

Purpose: Test actual ELLIOT orchestrator build_audit_graph() with ainvoke() using mocked external dependencies to isolate LangGraph execution from NIM API calls and browser automation.

Output: `elliot/tests/langgraph_investigation/test_02_full_audit_mocked.py` with full audit test, `conftest.py` with shared fixtures.
</objective>
...
# Codebase references
@C:/files/coding dev era/elliot\elliotAI/elliot/core/orchestrator.py (build_audit_graph, audit() method)
@C:/files/coding dev era\elliot\elliotAI/elliot/core/nim_client.py
@C:/files/coding dev era\elliot\elliotAI/elliot/agents/scout.py
@C:/files/coding dev era\elliot\elliotAI/elliot/tests/test_elliot.py
...
<task type="auto">
  ...
  <files>elliot/tests/langgraph_investigation/conftest.py</files>
  <action>
    Create `elliot/tests/langgraph_investigation/conftest.py` with shared investigation test fixtures:

    1. Imports: `pytest`, `unittest.mock.AsyncMock, MagicMock`, `dataclasses`, `sys`, `pathlib.Path`

    2. Add sys.path for elliot imports: `sys.path.insert(0, str(Path(__file__).resolve().parent.parent.parent))`
    ...
    Follow existing elliot test patterns for mocking and fixture structure.
  </action>
  <verify>
    python -c "from elliot.tests.langgraph_investigation.conftest import *; print('OK')"
  </verify>
  <done>
...
<task type="auto">
  ...
  <files>elliot/tests/langgraph_investigation/test_02_full_audit_mocked.py</files>
  <action>
    Create `elliot/tests/langgraph_investigation/test_02_full_audit_mocked.py` with full audit test:

    1. Imports: `pytest`, `asyncio`, `unittest.mock.patch, AsyncMock, MagicMock`, `dataclasses`, `sys`, `pathlib.Path`, `time`

    2. Add sys.path and import elliot modules:
    ...
    3. `@pytest.mark.asyncio` test `test_ainvoke_full_audit_mocked(mock_nim_client, mock_scout, audit_state)`:
       - Patch `elliot.agents.scout.StealthScout` to return mock_scout
       - Patch `elliot.core.nim_client.NIMClient` to return mock_nim_client
       - Patch `elliot.agents.vision.VisionAgent` with mocked analyze
    ...
    4. `@pytest.mark.asyncio` test `test_sequential_execution_fallback(mock_nim_client, mock_scout, audit_state)`:
       ...
       - Import `ElliotOrchestrator` from core.orchestrator
