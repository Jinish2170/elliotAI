...
depends_on: []
files_modified:
  - elliot/tests/langgraph_investigation/test_01_minimal_graph.py
autonomous: true
requirements:
  - CORE-03
  - CORE-03-2
...
<objective>
Create isolated minimal reproduction test to verify LangGraph ainvoke() behavior with Python 3.11+ asyncio, stripping away ELLIOT complexity to isolate CancelledError root cause.
...
Output: `elliot/tests/langgraph_investigation/test_01_minimal_graph.py` with minimal graph tests, `elliot/tests/langgraph_investigation/README.md` documentation.
</objective>
...
<task type="auto">
  ...
  <files>elliot/tests/langgraph_investigation/test_01_minimal_graph.py</files>
  <action>
    Create directory `elliot/tests/langgraph_investigation/` and file `test_01_minimal_graph.py` with minimal reproduction test:

    1. Imports: `asyncio`, `pytest`, `typing.TypedDict`, `langgraph.graph.StateGraph, END`, `sys`, `time`, `pathlib.Path`

    2. Add sys.path to import elliot modules: `sys.path.insert(0, str(Path(__file__).resolve().parent.parent.parent))`
    ...
    DO NOT use any ELLIOT agents or NIMClient - this is pure LangGraph isolation test.
  </action>
  <verify>
...
<task type="auto">
  ...
  <files>elliot/tests/langgraph_investigation/README.md</files>
  <action>
    Create `elliot/tests/langgraph_investigation/README.md` with:
  ...
  <verify>
    cat elliot/tests/langgraph_investigation/README.md
  </verify>
  <done>
