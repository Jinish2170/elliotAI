...
must_haves:
  ...
  artifacts:
    - path: "elliot/core/evidence_store.py"
      ...
      contains:
        ...
        - "raise RuntimeError"
  key_links:
    - from: "elliot/core/evidence_store.py:search_similar"
      to: "LanceDB.table_names()"
      via: "table existence check"
      pattern: "table_name not in self._db.table_names()"
    - from: "elliot/core/evidence_store.py:get_all_audits"
      to: "LanceDB.table_names()"
      via: "table existence check"
...
<task type="auto">
  ...
  <files>elliot/core/evidence_store.py</files>
  <action>
...
grep -rn "_json_search(" elliot/ --include="*.py"
grep -rn "_json_list_all(" elliot/ --include="*.py"
```
...
Return analysis summary before proceeding with Task 1 stub replacement.
  ...
  <verify>grep -rn "search_similar(" elliot/ --include="*.py" && grep -rn "get_all_audits(" elliot/ --include="*.py" && grep -rn "_json_search(" elliot/ --include="*.py" && grep -rn "_json_list_all(" elliot/ --include="*.py"</verify>
...
<task type="auto">
  ...
  <files>elliot/core/evidence_store.py</files>
  <action>
Modify elliot/core/evidence_store.py to replace empty return stubs with context-specific exceptions:
...
Include method name in error message for debugging context per research guidance.
  ...
  <verify>python -m pytest elliot/tests/test_elliot.py::TestEvidence -v</verify>
