...
must_haves:
  ...
  artifacts:
    - path: "elliot/tests/test_stub_cleanup.py"
  ...
  key_links:
    - from: "elliot/tests/test_stub_cleanup.py"
      to: "elliot/core/evidence_store.py"
      via: "import statements"
      pattern: "from core.evidence_store import EvidenceStore"
    - from: "elliot/tests/test_stub_cleanup.py"
      to: "elliot/agents/judge.py"
      via: "import statements"
    ...
    - from: "elliot/tests/test_stub_cleanup.py"
      to: "elliot/config/dark_patterns.py"
      via: "import statements"
...
Output: New test file elliot/tests/test_stub_cleanup.py with 10+ exception tests.
</objective>
...
@elliot/tests/test_security_dataclasses.py
</context>
...
<task type="auto">
  ...
  <files>elliot/tests/test_stub_cleanup.py</files>
  <action>
Create new test file elliot/tests/test_stub_cleanup.py following project patterns from test_ipc_queue.py and test_security_dataclasses.py.
...
Use mocking where needed to trigger exception conditions without external dependencies.
  ...
  <verify>python -m pytest elliot/tests/test_stub_cleanup.py -v</verify>
...
<verification>
1. Test file exists at elliot/tests/test_stub_cleanup.py
2. pytest.raises() used for all exception tests (per research pattern)
3. All 10+ tests pass: pytest elliot/tests/test_stub_cleanup.py -v
...
5. Overall test suite passes: pytest elliot/tests/ -v (all existing + new tests)
</success_criteria>
