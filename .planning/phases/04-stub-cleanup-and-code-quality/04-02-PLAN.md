  ...
  artifacts:
    ...
    - path: "elliot/config/dark_patterns.py"
      ...
      contains:
        ...
        - "raise ValueError"
  key_links:
    - from: "elliot/agents/judge.py:_summarize_dark_patterns"
      to: "evidence.vision_result"
      via: "null check"
      pattern: "if not evidence.vision_result"
    - from: "elliot/agents/judge.py:_summarize_entity_verification"
      to: "evidence.graph_result"
      via: "null check"
      pattern: "if not evidence.graph_result"
    - from: "elliot/config/dark_patterns.py:get_prompts_for_category"
...
<task type="auto">
  ...
  <files>elliot/agents/judge.py</files>
  <action>
...
grep -rn "_summarize_entity_verification(" elliot/ --include="*.py"
grep -rn "_check_dark_patterns_css(" elliot/ --include="*.py"
grep -rn "get_prompts_for_category(" elliot/ --include="*.py"
```
...
Return analysis summary before proceeding with Task 1 stub replacement.
  ...
  <verify>grep -rn "_summarize_dark_patterns(" elliot/ --include="*.py" && grep -rn "_summarize_entity_verification(" elliot/ --include="*.py" && grep -rn "_check_dark_patterns_css(" elliot/ --include="*.py" && grep -rn "get_prompts_for_category(" elliot/ --include="*.py"</verify>
...
<task type="auto">
  ...
  <files>elliot/agents/judge.py</files>
  <action>
Modify elliot/agents/judge.py to replace empty returns with RuntimeError:
...
Note: Line ~318 in dom_analyzer.py (_check_excessive_tracking) returns [] for valid edge case (tracking count <= 5) - do NOT modify as this is intentional behavior, not a stub.
  ...
  <verify>grep -n "def _summarize" elliot/agents/judge.py</verify>
...
<task type="auto">
  ...
  <files>elliot/analysis/dom_analyzer.py</files>
  <action>
Modify elliot/analysis/dom_analyzer.py to identify incomplete implementation:
...
Note: Line ~318 (_check_excessive_tracking) returns [] for valid edge case (tracking count <= 5) - do NOT modify as this is intentional behavior, not a stub.
  ...
  <verify>grep -n "_check_dark_patterns_css" elliot/analysis/dom_analyzer.py</verify>
...
<task type="auto">
  ...
  <files>elliot/config/dark_patterns.py</files>
  <action>
Modify elliot/config/dark_patterns.py to validate input:
...
Use ValueError for invalid input/input validation errors per research guidance.
  ...
  <verify>grep -n "def get_prompts_for_category" elliot/config/dark_patterns.py</verify>
