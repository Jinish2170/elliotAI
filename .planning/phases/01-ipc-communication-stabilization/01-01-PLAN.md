...
files_modified:
  ...
  - elliot/tests/test_ipc_queue.py
autonomous: true
requirements:
  - CORE-02
  - CORE-02-2
...
must_haves:
  ...
  artifacts:
    ...
    - path: elliot/tests/test_ipc_queue.py
...
Output: `elliot/core/ipc.py` module with ProgressEvent, Queue creation, and serialization functions; unit tests in `elliot/tests/test_ipc_queue.py`.
</objective>
...
<task type="auto">
  ...
  <action>
    Create new file `elliot/core/ipc.py` with:
  ...
  <verify>
    python -c "from elliot.core.ipc import ProgressEvent, create_queue, serialize_queue; print('OK')"
  </verify>
  <done>
...
<task type="auto">
  ...
  <files>elliot/tests/test_ipc_queue.py</files>
  <action>
    Create new file `elliot/tests/test_ipc_queue.py` with tests:
    ...
    Use pytest fixtures, mock environment variables as needed. Follow existing test patterns from `elliot/tests/test_elliot.py`.
  </action>
  <verify>
    cd elliot && python -m pytest tests/test_ipc_queue.py -v
  </verify>
  <done>
...
<success_criteria>
1. `elliot/core/ipc.py` module created with ProgressEvent and Queue utilities
2. `elliot/tests/test_ipc_queue.py` created with 5 passing unit tests
