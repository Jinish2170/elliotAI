...
must_haves:
  truths:
    - ElliotOrchestrator accepts optional progress_queue parameter
  ...
  artifacts:
    - path: elliot/core/orchestrator.py
      contains: ElliotOrchestrator class with progress_queue parameter
      contains: _emit() method with dual-mode support
    - path: elliot/core/ipc.py
      provides: safe_put helper for Queue with timeout handling
      exports: safe_put
  key_links:
    - from: elliot/core/orchestrator.py
      to: elliot/core/ipc.py
      via: from elliot.core.ipc import ProgressEvent, safe_put
      pattern: from elliot\.core\.ipc import ProgressEvent
    - from: elliot/core/orchestrator.py
      to: progress_queue
      via: queue.put()
...
<objective>
Modify ElliotOrchestrator to support dual-mode emission via Queue or stdout based on progress_queue parameter availability.
...
Output: Modified `elliot/core/orchestrator.py` with dual-mode _emit(), and safe_put helper in `elliot/core/ipc.py`.
</objective>
...
# Codebase references
@C:/files/coding dev era/elliot/elliotAI/elliot/core/orchestrator.py
@C:/files/coding dev era/elliot/elliotAI/elliot/core/ipc.py (created in Plan 01)
...
<task type="auto">
  ...
  <files>elliot/core/ipc.py</files>
  <action>
    Extend `elliot/core/ipc.py` with safe_put helper:
  ...
  <verify>
    python -c "
from elliot.core.ipc import create_queue, safe_put
import logging
...
<task type="auto">
  <name>Modify ElliotOrchestrator to accept progress_queue and implement dual-mode emission</name>
  <files>elliot/core/orchestrator.py</files>
  <action>
    Modify `elliot/core/orchestrator.py` with dual-mode emission:
    ...
    2. Modify `ElliotOrchestrator.__init__` to accept progress_queue:
       ```python
...
# Test with queue
from elliot.core.ipc import create_queue
q = create_queue(maxsize=100)
orch2 = ElliotOrchestrator(progress_queue=q)
assert orch2.progress_queue is q
print('OK - queue mode works')
"
  </verify>
  <done>
    ElliotOrchestrator accepts progress_queue parameter, emits via Queue when available, falls back to stdout when None, all _emit() calls updated.
  </done>
</task>
...
<verification>
1. ElliotOrchestrator.__init__ accepts progress_queue parameter
...
<success_criteria>
1. ElliotOrchestrator accepts optional progress_queue parameter
