...
must_haves:
  ...
  key_links:
    - from: elliot/__main__.py
      to: elliot/core/ipc.py
      via: from elliot.core.ipc import determine_ipc_mode
      pattern: from elliot\.core\.ipc import determine_ipc_mode
    - from: elliot/core/ipc.py
      to: os.environ
...
Output: CLI arguments parsed in `elliot/__main__.py`, mode determination logic in `elliot/core/ipc.py`.
</objective>
...
# Codebase references
@C:/files/coding dev era/elliot/elliotAI/elliot/__main__.py
...
<task type="auto">
  ...
  <action>
    Extend `elliot/core/ipc.py` (created in Plan 01) with mode determination:
...
from elliot.core.ipc import determine_ipc_mode, IPC_MODE_QUEUE
...
<task type="auto">
  <name>Add CLI flags to elliot/__main__.py for IPC mode control</name>
  <files>elliot/__main__.py</files>
  <action>
    Modify `elliot/__main__.py` to add CLI flags:
    ...
    2. After `args = parser.parse_args()`, add IPC mode determination:
       ```python
       from elliot.core.ipc import determine_ipc_mode, IPC_MODE_QUEUE, create_queue, deserialize_queue_from_env
  ...
  <verify>
    python -m elliot https://example.com --help 2>&1 | grep -E "(use-queue-ipc|use-stdout|validate-ipc)"
  </verify>
  <done>
...
<success_criteria>
1. CLI arguments added to elliot/__main__.py for IPC mode control
2. Mode determination logic added to elliot/core/ipc.py with correct priority
