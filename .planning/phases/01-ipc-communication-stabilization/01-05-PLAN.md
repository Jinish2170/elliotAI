...
depends_on:
  - 01-04
files_modified:
  - elliot/tests/test_ipc_integration.py
...
must_haves:
  ...
  artifacts:
    - path: elliot/tests/test_ipc_integration.py
    ...
    - path: backend/tests/test_audit_runner_queue.py
      ...
      min_lines: 60
  key_links:
    - from: elliot/tests/test_ipc_integration.py
      to: elliot/core/orchestrator.py
      via: from elliot.core.orchestrator import ElliotOrchestrator
      pattern: from elliot\.core\.orchestrator import
...
# Codebase references
@C:/files/coding dev era/elliot/elliotAI/elliot/tests/test_elliot.py (test patterns)
@C:/files/coding dev era/elliot/elliotAI/elliot/core/ipc.py (created in Plan 01)
@C:/files/coding dev era/elliot/elliotAI/elliot/core/orchestrator.py (modified in Plan 03)
...
<task type="auto">
  ...
  <files>elliot/tests/test_ipc_integration.py</files>
  <action>
    Create new file `elliot/tests/test_ipc_integration.py` with tests:
    ...
    2. `test_orchestrator_with_queue_mode()` test:
       ...
       - Create ElliotOrchestrator(progress_queue=queue)
    ...
    3. `test_orchestrator_stdout_fallback()` test:
       - Create ElliotOrchestrator(progress_queue=None)
       - Run minimal audit
    ...
    4. `test_queue_full_handling()` test:
       ...
       - Create ElliotOrchestrator with small Queue
    ...
    Use mocks for NIM client to avoid external dependencies. Follow existing test patterns from `test_elliot.py`.
  </action>
  <verify>
    cd elliot && python -m pytest tests/test_ipc_integration.py -v -x
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
    ...
    3. File content:
       ...
       # Add backend and elliot to path
       sys.path.insert(0, str(Path(__file__).resolve().parent.parent.parent))
       sys.path.insert(0, str(Path(__file__).resolve().parent.parent.parent / "elliot"))
