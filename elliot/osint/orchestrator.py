...
from elliot.osint.types import OSINTCategory, OSINTResult, SourceConfig


logger = logging.getLogger("elliot.osint.orchestrator")
...
class OSINTOrchestrator:
    ...
    def __init__(self, db_session=None):
        ...
        # Per-audit query dedup cache. Keyed by (source, method, args-repr); value
        # is the resolved OSINTResult (or None for known-no-result). Prevents
        # repeating the same DNS/WHOIS/SSL/threat-intel lookup N times when the
        # same indicator (IP, domain, hash) appears on multiple pages. Cleared
        # between audits via reset_audit_cache().
        self._query_cache: Dict[tuple, Optional[OSINTResult]] = {}

        # Auto-discover available sources
        self._discover_sources()

    def reset_audit_cache(self) -> None:
        """Clear per-audit dedup cache. Call at the start of each new audit."""
        n = len(self._query_cache)
        self._query_cache.clear()
        if n:
            logger.debug(f"OSINTOrchestrator: cleared {n} cached query entries")

    @staticmethod
    def _make_cache_key(source_name: str, method: str, args: tuple, kwargs: dict) -> tuple:
        """Deterministic key for the dedup cache.

        repr() is used for stability; OSINT queries pass primitives (strings,
        ints), so this is well-behaved. Sorted kwargs for order-independence.
        """
        return (source_name, method, tuple(repr(a) for a in args),
    ...
    def _discover_sources(self) -> None:
        ...
        import importlib
        _darknet_sources = [
            ("elliot.osint.sources.darknet_alpha", "AlphaBayMarketplaceSource"),
            ("elliot.osint.sources.darknet_hansa", "HansaMarketplaceSource"),
            ("elliot.osint.sources.darknet_empire", "EmpireMarketplaceSource"),
            ("elliot.osint.sources.darknet_dream", "DreamMarketplaceSource"),
            ("elliot.osint.sources.darknet_wallstreet", "WallStreetMarketplaceSource"),
            ("elliot.osint.sources.darknet_tor2web", "Tor2WebDeanonSource"),
        ]
        ...
        if hasattr(settings, "TAVILY_API_KEY") and settings.TAVILY_API_KEY:
            try:
                from elliot.osint.sources.tavily_source import (
    ...
    async def query_with_retry(
        ...
        **kwargs
        ...
        # Dedup: same source+method+args within this audit returns the cached
        # OSINTResult. Saves repeat network calls when the same IOC (IP, domain,
        # hash) is mentioned on multiple pages.
        cache_key = self._make_cache_key(source_name, method, args, kwargs)
        if cache_key in self._query_cache:
            cached = self._query_cache[cache_key]
            logger.debug(
                f"query_with_retry: dedup hit for '{source_name}' "
                f"(method={method}, args={args!r})"
            )
            return cached
