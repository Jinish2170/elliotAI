...
if vision_module_path.exists():
    ...
    spec = importlib.util.spec_from_file_location('elliot.agents.vision_module', str(vision_module_path))
    vision_module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(vision_module)
    sys.modules['elliot.agents.vision_module'] = vision_module
