"""
Elliot Agent 3 — The Cross-Examiner (Graph Reasoning)

The "Detective" of Elliot. Verifies entities extracted from the website
...
from elliot.osint.reputation import ReputationManager, VerdictType, get_reputation_manager
from elliot.osint.cti import CThreatIntelligence
from elliot.quality.consensus_engine import ConsensusEngine

logger = logging.getLogger("elliot.graph")
...
class GraphInvestigator:
    ...
    def __init__(self, nim_client: Optional[NIMClient] = None, db_session=None):
        ...
        # Use the process-wide reputation singleton so source-accuracy data
        # accumulates across audits via the JSON-backed store. begin_session()
        # is called when a new audit's investigation kicks off so per-audit
        # prediction ledgers don't leak between runs.
        self._osint_orchestrator = OSINTOrchestrator(self._db_session)
        self._reputation_manager = get_reputation_manager()
        self._reputation_manager.begin_session()
    ...
    def _record_source_prediction(self, source_name: str, result) -> None:
        """Log this source's prediction to the ReputationManager.

        Maps OSINTResult to VerdictType conservatively: only emits MALICIOUS or
        SAFE when the result data carries an explicit signal. Everything else
        is recorded as UNKNOWN so calibration data still accumulates without
        biasing accuracy stats.
        """
        try:
            from elliot.osint.reputation import VerdictType
            data = getattr(result, "data", None) or {}
            confidence = float(getattr(result, "confidence_score", 0.0) or 0.0)

            # Look for common malicious/safe flags. Sources are heterogeneous;
            # we err on the side of UNKNOWN rather than fabricating a verdict.
            verdict = VerdictType.UNKNOWN
            if isinstance(data, dict):
                explicit = (
                    data.get("verdict")
                    or data.get("threat_verdict")
                    or data.get("classification")
                )
                if isinstance(explicit, str):
                    e = explicit.lower()
                    if e in ("malicious", "phishing", "malware", "fraud"):
                        verdict = VerdictType.MALICIOUS
                    elif e in ("safe", "clean", "benign"):
                        verdict = VerdictType.SAFE
                    elif e in ("suspicious", "warning"):
                        verdict = VerdictType.SUSPICIOUS
                # Numeric threat scores (AbuseIPDB style: 0..100)
                if verdict == VerdictType.UNKNOWN:
                    score = data.get("abuse_confidence_score") or data.get("threat_score")
            ...
            self._reputation_manager.record_prediction(
                source=source_name,
                verdict=verdict,
                confidence=confidence if 0.0 <= confidence <= 1.0 else 0.5,
            )
        except Exception as e:
            # Never let reputation bookkeeping break the audit.
            logger.debug(f"record_source_prediction failed for {source_name}: {e}")
    ...
    async def _run_osint_investigation(
        ...
        # Clear per-audit dedup cache before this run. A single audit may call
        # _run_osint_investigation multiple times (one per iteration / re-scan);
        # clearing here would defeat dedup, so we only reset on construction —
        # the orchestrator instance is per-audit via GraphInvestigator.
        ...
        if domain and results:
            try:
                from elliot.osint.types import OSINTResult
                ...
                for source_name, result in threat_results.items():
                    if isinstance(result, OSINTResult) and result.status.value == "success":
                        ...
                        # Record this source's prediction so its accuracy can be
                        # tracked over time. Mapping is intentionally conservative:
                        # we only record a non-UNKNOWN verdict when the source
                        # itself flagged the indicator (threat_score / verdict /
                        # malicious flag). Without persistent storage, this is
                        # session-local — closing the loop with record_actual()
                        # after the Judge renders the final verdict is the next
                        # step but requires DB-backed reputation.
                        self._record_source_prediction(source_name, result)
            except Exception as e:
        ...
        if results and self._consensus_engine:
            try:
                ...
                from elliot.osint.types import OSINTResult, SourceStatus, OSINTCategory
    ...
    async def _tavily_search(self, query: str) -> list[dict]:
        ...
        try:
            from elliot.core.web_searcher import web_search
            return await self._run_with_timeout(
                web_search(
                    query=query,
                    max_results=5,
    ...
    def _ip_geolocation_sync(self, ip: str) -> dict:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "Elliot/1.0"})
