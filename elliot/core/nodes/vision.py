...
async def vision_node(state: AuditState) -> dict:
    ...
    try:
        ...
        # Hard pre-call cap inside NIMClient (defense-in-depth). The Vision agent
        # also self-throttles via call_count loops, but a single NIMClient gate
        # means a misbehaving pass or future parallel branch can't overrun.
        nim.set_budget(nim_budget)
