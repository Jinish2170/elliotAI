"""
Elliot Core — NVIDIA NIM API Client
...
logger = logging.getLogger("elliot.nim")
...
class NIMBudgetExceeded(Exception):
    """Per-audit NIM call budget reached; caller must degrade/skip."""
    pass
...
class NIMClient:
    ...
    def __init__(self):
        ...
        # Hard pre-call budget cap (None = unlimited). Set via set_budget() by the
        # orchestrator/agent that owns this client for the current audit. Enforced
        # atomically before _call_count is incremented so over-spend is impossible.
        self._budget_limit: Optional[int] = None

        # Lazy-init the client (allows creation without API key for testing)
        self._initialized = False

    def set_budget(self, limit: Optional[int]) -> None:
        """Set hard per-client NIM call budget. None disables the cap."""
        self._budget_limit = limit if (limit is None or limit > 0) else 0

    def budget_remaining(self) -> Optional[int]:
        """Calls remaining before NIMBudgetExceeded is raised. None = unlimited."""
        if self._budget_limit is None:
            return None
        return max(0, self._budget_limit - self._call_count)
    ...
    async def _aread_image(self, image_path: str) -> bytes:
        """Async-safe read of an image file (off the event loop)."""
        return await asyncio.to_thread(self._read_image_bytes, image_path)
    ...
    async def analyze_image(
        ...
        if pass_type is not None:
            # get_cache_key reads the image file; offload to a thread.
            cache_key = await asyncio.to_thread(
                self.get_cache_key, image_path, prompt, pass_type
            )
        else:
        ...
        if result:
            ...
            await asyncio.to_thread(self._write_cache, cache_key, result)
            return {**result, "cached": False}
    ...
    async def generate_text(
        ...
        try:
            ...
            await asyncio.to_thread(self._write_cache, cache_key, output)
            return {**output, "cached": False}

        except NIMBudgetExceeded as e:
            # Budget gate: surface a degraded result without recording a "failure"
            # (it's a hard cap, not an outage), and skip the circuit breaker.
            logger.warning(str(e))
            return {
                "response": f"[NIM_BUDGET_EXCEEDED] {e}",
                "model": "none",
                "fallback_mode": True,
                "cached": False,
    ...
    async def _try_vision_model(
        ...
        try:
            # Read + base64-encode off the event loop. Screenshots routinely run
            # 500KB-2MB; doing this inline blocks the asyncio scheduler for tens
            # of ms and starves other concurrent agents.
            image_bytes = await self._aread_image(image_path)
            ...
            # Hard budget cap. Don't pollute _errors or trip the circuit breaker —
            # this is an intentional stop, not a service failure.
            logger.warning(f"VLM call to {model} skipped: {e}")
            return None
        except Exception as e:
    ...
    async def _rate_limited_call(
        ...
        async with self._semaphore:
            # Pre-call budget gate. Atomic with the increment below: once we hold
            # the semaphore, no other coroutine can race past this check.
            if self._budget_limit is not None and self._call_count >= self._budget_limit:
                raise NIMBudgetExceeded(
                    f"NIM budget exhausted: {self._call_count}/{self._budget_limit} calls used"
                )
            self._call_count += 1
