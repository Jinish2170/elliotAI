"""
Elliot Core — LangGraph Orchestrator
...
from elliot.agents.graph_investigator import GraphInvestigator, GraphResult
from elliot.agents.judge import AuditEvidence, JudgeAgent, JudgeDecision
from elliot.agents.scout import ScoutResult, StealthScout
from elliot.agents.vision import VisionAgent, VisionResult
from elliot.config import settings
from elliot.config.trust_weights import TrustScoreResult
from elliot.core.complexity_analyzer import ComplexityAnalyzer
from elliot.core.degradation import DegradedResult, FallbackManager, FallbackMode
from elliot.core.ipc import ProgressEvent, SecurityModeCompleted, SecurityModeStarted, safe_put
from elliot.core.nim_client import NIMClient
from elliot.core.timeout_manager import ComplexityMetrics, TimeoutManager, TimeoutStrategy
...
# ============================================================
# ============================================================
# Node Functions — imported from elliot.core.nodes package
# ============================================================
# AuditState is defined above, so nodes can safely import it
from elliot.core.nodes import (  # noqa: E402
    scout_node,
    vision_node,
    graph_node,
    judge_node,
...
from elliot.core.nodes.security import _get_security_modules_for_tier  # noqa: E402
...
def build_audit_graph() -> StateGraph:
    """
    Build the LangGraph state machine for a Elliot audit.
...
class ElliotOrchestrator:
    """
    High-level API for running a Elliot audit.
    ...
    def __init__(
        ...
        if use_progress_streaming:
            from elliot.core.progress import ProgressEmitter, CompletionTimeEstimator
        ...
        # NOTE: quality penalty is tracked per-audit in state["_quality_penalty"],
        # not as instance state, so the same orchestrator can run concurrent audits
    ...
    def _register_fallback_functions(self):
        ...
        async def judge_fallback(**kwargs) -> dict:
            ...
            from elliot.config.trust_weights import SubSignal, compute_trust_score
    ...
    async def _execute_agent_smart(
        ...
        if self.use_circuit_breaker and self._fallback_manager is not None:
            ...
            if degraded_result.is_degraded():
                ...
                state["_quality_penalty"] = state.get("_quality_penalty", 0.0) + quality_penalty
    ...
    async def audit(
        ...
        logger.info(f"Starting Elliot audit: {url} | tier={tier} | max_pages={tier_config['pages']}")
        ...
        try:
            for iteration in range(state["max_iterations"]):
                ...
                try:
                    ...
                    # Note: vision_node returns the cumulative nim_calls_used in its
                    # update dict, so state.update above already carries it forward.
            ...
            # Close the OSINT reputation feedback loop. Map the Judge's final
            # risk_level to a coarse VerdictType and retro-grade every OSINT
            # prediction recorded during this audit. State accumulates across
            # audits via the JSON-backed reputation store, so over time
            # accurate sources earn higher consensus weight while noisy ones
            # are penalized — without needing a manual labeling step.
            try:
                jd = state.get("judge_decision") or {}
                tr = jd.get("trust_score_result") if isinstance(jd, dict) else None
                final_risk = (
                    tr.get("risk_level") if isinstance(tr, dict) else None
                )
                if final_risk:
                    from elliot.osint.reputation import (
                        VerdictType,
                        get_reputation_manager,
                    )
                    risk_str = str(final_risk).lower()
                    if risk_str in ("critical", "high", "high_risk",
                                    "dangerous", "likely_fraudulent"):
                        final_verdict = VerdictType.MALICIOUS
                    elif risk_str in ("suspicious", "moderate"):
                        final_verdict = VerdictType.SUSPICIOUS
                    elif risk_str in ("low", "low_risk", "safe", "trusted",
                                      "probably_safe"):
                        final_verdict = VerdictType.SAFE
                    else:
                        final_verdict = VerdictType.UNKNOWN
                    graded = get_reputation_manager().apply_session_verdict(final_verdict)
                    if graded:
                        logger.info(
                            f"Reputation: graded {graded} OSINT predictions "
                            f"against final verdict={final_verdict.value}"
                        )
            except Exception as rep_err:
                logger.warning(f"Reputation feedback loop failed: {rep_err}")

            # Apply quality penalty to final trust score
            quality_penalty = state.get("_quality_penalty", 0.0)
            ...
            try:
                ...
                if jd and isinstance(jd, dict):
                    ...
                    if cvss_dict:
                        ...
                        # CVSS v3.1 vector string + qualitative band (new fields
                        # produced by elliot.cwe.cvss_v31.build_cvss_report).
                        # SIEMs / ticket systems consume the vector string directly.
                        if "vector_string" in cvss_dict:
                            sec_res["cvss_vector_string"] = cvss_dict["vector_string"]
                        if "severity" in cvss_dict:
                            sec_res["cvss_severity_band"] = cvss_dict["severity"]
            ...
            # Emit error + flush buffered events so the frontend isn't left mid-progress.
            # Without this flush, buffered findings and the final "failed" status can be
            # silently dropped when the orchestrator aborts.
            if self.use_progress_streaming and self._progress_emitter:
                try:
                    ...
                    logger.warning(f"ProgressEmitter flush during error path failed: {flush_err}")
