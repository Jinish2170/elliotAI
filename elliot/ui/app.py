"""
Elliot UI v3 — Masterclass Forensic Audit Dashboard
...
from elliot.config.dark_patterns import DARK_PATTERN_TAXONOMY
from elliot.ui.darknet_components import (
...
logger = logging.getLogger("elliot.ui")
...
st.set_page_config(
    page_title="Elliot — Forensic Web Auditor",
    page_icon="🛡️",
    layout="wide",
...
def _run_audit_streaming(url: str, tier: str, progress_container, log_container,
    ...
    """
    elliot_root = Path(__file__).resolve().parent.parent
    ...
    cmd = [python_exe, "-m", "elliot", url, "--tier", tier, "--output", tmp_path,
    ...
    try:
        proc = subprocess.Popen(
            cmd, cwd=str(elliot_root.parent),
...
st.markdown("# 🛡️ Elliot — Forensic Web Auditor")
    ...
    if result.get("status") == "error":
        ...
        with c2:
            st.download_button("📄 Raw JSON", data=json.dumps(result, indent=2, default=str),
                               file_name="elliot_error.json", mime="application/json")
        ...
        with tab_raw:
            ...
            with dc1:
                st.download_button("📄 Download JSON",
                                   ...
                                   file_name=f"elliot_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
...
# ============================================================
    ...
    st.markdown("---")
    st.caption("Elliot v4.0 — Autonomous Multi-Modal Forensic Web Auditor · Built with NVIDIA NIM, LangGraph, Playwright & Streamlit")
