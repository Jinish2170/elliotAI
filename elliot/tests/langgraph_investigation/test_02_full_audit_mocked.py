"""
Elliot LangGraph Investigation — Full Audit Test with Mocked Dependencies

Tests actual ELLIOT orchestrator build_audit_graph() with ainvoke() using mocked
...
# Import elliot modules (using actual orchestrator)
from elliot.core.orchestrator import (
    AuditState,
    build_audit_graph,
    judge_node,
    route_after_judge,
...
async def test_ainvoke_full_audit_mocked(
    ...
    with patch("elliot.core.orchestrator.NIMClient", return_value=mock_nim_client), \
         ...
         patch("elliot.agents.vision.VisionAgent.__init__", return_value=None), \
         patch("elliot.agents.vision.VisionAgent.analyze", side_effect=mock_vision_agent), \
         patch("elliot.agents.graph_investigator.GraphInvestigator.__init__", return_value=None), \
         patch("elliot.agents.graph_investigator.GraphInvestigator.investigate", side_effect=mock_graph_investigator), \
         patch("elliot.agents.judge.JudgeAgent.__init__", return_value=None), \
         patch("elliot.agents.judge.JudgeAgent.deliberate", side_effect=mock_judge_agent), \
         patch("elliot.agents.security_agent.SecurityAgent.initialize", return_value=None), \
         patch("elliot.agents.security_agent.SecurityAgent.analyze", return_value={"composite_score": 0.7, "findings": [], "total_findings": 0, "modules_run": [], "modules_failed": 0, "analysis_time_ms": 0, "modules_results": {}, "errors": []}):
...
async def test_sequential_execution_fallback(
    ...
    """
    Test sequential execution via ElliotOrchestrator.audit() method.
    ...
    with patch("elliot.core.orchestrator.NIMClient", return_value=mock_nim_client), \
        ...
        from core.orchestrator import ElliotOrchestrator

        # Create orchestrator instance
        orchestrator = ElliotOrchestrator(progress_queue=None)
...
async def test_security_node_isolated(mock_nim_client: MagicMock):
    ...
    with patch("elliot.core.orchestrator.NIMClient", return_value=mock_nim_client), \
         patch("elliot.analysis.security_headers.SecurityHeaderAnalyzer") as mock_headers, \
         patch("elliot.analysis.phishing_checker.PhishingChecker") as mock_phishing:
...
async def test_judge_node_isolated():
    ...
    with patch("elliot.core.orchestrator.NIMClient"), \
         patch("elliot.agents.judge.JudgeAgent") as mock_judge:
