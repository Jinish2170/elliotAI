"""
Elliot LangGraph Investigation — Behavioral Differences Test
...
# Add elliot root to path for imports
...
# Import ELLIOT modules
from elliot.core.orchestrator import build_audit_graph
...
async def simple_async_node(state: ObservableState) -> dict:
    ...
    Used for behavioral observation without ELLIOT complexity.
    """
...
# ============================================================
# Test 4: ELLIOT Graph Structure Validation
# ============================================================

def test_elliot_graph_structure_validation():
    """
    Validate ELLIOT graph structure exists and is correct.

    This is a sanity check to ensure the full ELLIOT graph can be built,
    ...
    assert graph is not None, "ELLIOT graph should build successfully"

    compiled = graph.compile()
    assert compiled is not None, "ELLIOT graph should compile successfully"
    ...
    print(f"\n✓ ELLIOT graph structure validated: {len(expected_nodes)} nodes present")
...
async def test_state_mutation_patterns():
    """
    Test state mutation patterns used in ELLIOT nodes.

    ELLIOT nodes update state with dicts like:
    ...
    # ELLIOT-style state update
    async def elliot_style_node(state: ObservableState) -> dict:
        await asyncio.sleep(0.001)
    ...
    graph = StateGraph(ObservableState)
    graph.add_node("v1", observable_node(logger, "v1", elliot_style_node))
    graph.add_node("v2", observable_node(logger, "v2", elliot_style_node))
    ...
    print(f"\n✓ ELLIOT-style state mutations work correctly")
...
async def test_async_context_manager_pattern():
    """
    Test async context manager pattern (used by Scout in ELLIOT).

    ELLIOT Scout node uses:
...
# ============================================================
# Test 7: ELLIOT Full Graph Timeout Behavior
# ============================================================

@pytest.mark.asyncio
async def test_elliot_full_graph_timeout_behavior():
    """
    Document ELLIOT full graph timeout/hanging behavior.

    This test acknowledges the finding from Phase 02 that the full
    ELLIOT graph hangs or times out via ainvoke, and confirms that
    ...
    print(f"\n⚠ ELLIOT full graph ainvoke() known to hang (Phase 02 finding)")
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        print("\n--- Test 7: ELLIOT full graph behavior ---")
        try:
            await test_elliot_full_graph_timeout_behavior()
        except Exception as e:
            print(f"FAILED: {e}")
