...
from elliot.core.types import SecurityResult, SecurityFinding, Severity, SecurityConfig
...
class TestSecurityAgentInit:
    ...
    async def test_init_defaults_to_tier_execution_when_available(self, mock_nim_client):
        ...
        with patch("elliot.agents.security_agent.TIER_AVAILABLE", True):
            with patch("elliot.agents.security_agent.SECURITY_USE_TIER_EXECUTION", True):
                agent = SecurityAgent(nim_client=mock_nim_client)
    ...
    async def test_init_falls_back_when_tier_execution_unavailable(self, mock_nim_client):
        ...
        with patch("elliot.agents.security_agent.TIER_AVAILABLE", False):
            with patch("elliot.agents.security_agent.SECURITY_USE_TIER_EXECUTION", True):
                agent = SecurityAgent(nim_client=mock_nim_client)
